...
namespace Tests
{
    ...
    public class VkTests
    {
        ...
        public void GetSongPairs()
        {
            ...
            var handler = SpotifyHandler.GetAuthorizedByIds(_clientId, _secretId);

            var tracksPairs = handler.FindTracksPairs(trackList, 50);
            ...
            foreach (var pair in tracksPairs)
            {
                i++;
                sb.Append(
                    $"{i}. VK: {pair.Key.Artist} - {pair.Key.Title} | ID:{pair.Value.Id} Spotify: {pair.Value.Artist} - {pair.Value.Title}, " +
